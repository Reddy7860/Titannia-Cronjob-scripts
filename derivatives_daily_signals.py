...
for idx in range(0,len(symbol_list)):
    ...
    # Set the threshold values for the signals
    ...
    for i in range(1, len(futures_data)):
        ...
        elif (futures_data.iloc[i-1]['Signal'] == 'Long Unwinding' and price_change_pct > 0) or \
             (futures_data.iloc[i-1]['Signal'] == 'Short Covering' and price_change_pct < 0):
...
collection = db["Derivatives_Daily_Signals"]
